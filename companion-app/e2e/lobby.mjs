...
const CHANNEL = process.env.KANAL ?? 'lobby-e2e'
const sleep = (ms) => new Promise((r) => setTimeout(r, ms))

function launch(name, port) {
  return spawn(electronPath, [
    '.', `--remote-debugging-port=${port}`,
    `--user-data-dir=${mkdtempSync(join(tmpdir(), `ts3l-${name}-`))}`,
    '--ts3-server=ts.test.pl:9987', `--channel=${CHANNEL}`,
    '--signaling=ws://127.0.0.1:8080',
    ...(process.env[`NICK_${name.toUpperCase()}`]
      ? [`--nick=${process.env[`NICK_${name.toUpperCase()}`]}`] : [])
...
const HEADER = `document.querySelector('.app__stage-tag').textContent.trim()`
const PANEL = `(() => {
  const items = [...document.querySelectorAll('.peers__item')].map(li => ({
    text: li.querySelector('.peers__name').textContent.trim(),
    streaming: li.classList.contains('peers__item--streaming')
  }));
  const collapsed = !!document.querySelector('.peers--collapsed');
  return JSON.stringify({ collapsed, items });
})()`
const TOGGLE_PANEL = `document.querySelector('.peers__toggle').click()`
const SHARE_BUTTON = `(() => {
...
const IMAGE = `(async () => {
  ...
  if (!v || !v.videoWidth) return JSON.stringify({ image: false,
    text: (document.querySelector('.viewer__waiting')||{}).textContent || '' });
  ...
  return JSON.stringify({ image: true, size: v.videoWidth+'x'+v.videoHeight,
    uniqueFrames: new Set(h).size })
})()`

/** Klika "Udostępnij ekran", wybiera pierwszy ekran i potwierdza. */
async function share(cdp, withAudio = false) {
...
try {
  ...
  console.log('L1 A naglowek:', await A.evaluate(HEADER))
  console.log('L1 B przycisk:', await B.evaluate(SHARE_BUTTON))

  console.log('L2 A udostepnia:', await share(A))
  await sleep(7000)
  console.log('L2 A naglowek :', await A.evaluate(HEADER))
  console.log('L2 B naglowek :', await B.evaluate(HEADER))
  console.log('L2 B przycisk :', await B.evaluate(SHARE_BUTTON))
  console.log('L2 B obraz    :', await B.evaluate(IMAGE))
  ...
  await sleep(8000)
  console.log('L3 C (dolaczyl w trakcie):', await C.evaluate(IMAGE))
  console.log('L3 panel u C  :', await C.evaluate(PANEL))
  await B.evaluate(TOGGLE_PANEL)
  await sleep(600)
  ...
  await sleep(4000)
  console.log('L4 B naglowek :', await B.evaluate(HEADER))
  console.log('L4 B przycisk :', await B.evaluate(SHARE_BUTTON))

  // Teraz B przejmuje nadawanie — to jest sedno "kazdy moze byc streamerem"
  console.log('L5 B udostepnia (z dzwiekiem):', await share(B, true))
  await sleep(7000)
  console.log('L5 A naglowek :', await A.evaluate(HEADER))
  console.log('L5 A obraz    :', await A.evaluate(IMAGE))
  console.log('L5 C obraz    :', await C.evaluate(IMAGE))
  console.log('L5 panel u C  :', await C.evaluate(PANEL))

  // --- L6: DRUGI nadajacy rownoczesnie (wczesniej serwer by go odrzucil)
  console.log('L6 A dolacza jako drugi nadajacy:', await share(A))
  await sleep(9000)
  console.log('L6 siatka u C :', await C.evaluate(GRID))
  console.log('L6 siatka u A :', await A.evaluate(GRID))
  ...
  const FRAMES = `(async () => {
    ...
    for (const el of document.querySelectorAll('.tile')) {
      const v = el.querySelector('.tile__video');
      const name = el.querySelector('.tile__name').textContent.trim();
      if (!v || !v.videoWidth) { out.push({ name, image: 'BRAK' }); continue; }
      ...
      let sum=0; const d=ctx.getImageData(0,0,120,68).data;
      for (let q=0;q<d.length;q+=4) sum+=d[q];
      out.push({ name, image: v.videoWidth+'x'+v.videoHeight,
        unique: h.size, black: sum === 0 });
    }
    return JSON.stringify(out);
  })()`
  console.log('L6b klatki u C:', await C.evaluate(FRAMES))
  console.log('L6b klatki u A:', await A.evaluate(FRAMES))
  console.log('L6b klatki u B:', await B.evaluate(FRAMES))

  // --- L7: jeden konczy, drugi ma nadawac dalej
  await A.evaluate(`[...document.querySelectorAll('button')].find(x=>/Zakończ udostępnianie/.test(x.textContent)).click()`)
  await sleep(5000)
  console.log('L7 siatka u C po zakonczeniu A:', await C.evaluate(GRID))
